...
- **The `failed-tasks` check drains `failed/` monotonically; recursion is guarded by
  ...
  the second cycle: a failed task whose `data["body"]` carries
  ...
  again. So a `heal-declined` in `healed/` means "the automated fix didn't
