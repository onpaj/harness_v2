...
- **The `failed-tasks` check drains `failed/` monotonically; recursion is guarded by
  a marker, not by construction.** The check claims each failed task once and settles
  it to `healed/`, stamping `data.heal` on the fresh heal task it produces. A heal
  ...
  filed, and is settled with a `heal-declined` note instead of being healed
  ...
  `GithubMergeableCheck`), and is settled with a `heal-declined` note the same
  way. So a `heal-declined` in `healed/` means "the automated fix didn't
  work, it's yours now" — not an error.
