...
- `behaviors/unblock_pr.py` — `UnblockPrBehavior`: merges the base into the
  ...
  outcome with a *clean* merge still commits: there is no merge to abandon and
  the branch carries the agent's own work in progress. Excluding `.artifacts`
...
- **A Process** is the operator's top-level authoring aggregate — the "tie it all
  ...
  **trigger** (cadence), an **action** (a named `Check` + params — the inbound
  ...
  workflow's `dry_run`) and `unblock-pr.json` (ADR-0026, scanning every open PR
  of every registered repo); the operator adds any others, each an independent
  file.
