...
- **The `failed-tasks` check drains `failed/` of everything healable — not to
  ...
  `"mergeability"` or `"pull-request"` is a resolver or automerge-review task
  minted from the harness's own open pull request (`GithubConflictsCheck`/
  `GithubMergeableCheck`), and gets a `heal-declined` note the same
