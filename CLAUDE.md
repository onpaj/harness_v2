...
writes into a step queue. `TaskControl` is touched only by `TaskControlService` (core), `api/` and wiring — `dispatcher.py`/`consumer.py` don't import it; guarded by `test_architecture.py`. See ADR-0011 and ADR-0012.
24. **`failed/` has one reader — the `failed-tasks` Check — and there are exactly two terminal columns, `done` and `failed`.** This refines "terminal states are queues nobody consumes": `done`/`end` are terminal, while `failed/` is drained by the `failed-tasks` Check (an action of `processes/autoheal.json`, which `harness init` seeds on every root; its `action.params.repository` is the only place self-healing names a repo) and by nothing else. The router and dispatcher never learn about `failed` as a step. It drains of everything *healable*: a claimed failure is retired into `done/` with `status = END` — the same terminal an ordinary completion reaches, because nobody has to do anything about it any more — and the healer's involvement is recorded in the task's *history* (the `failed-tasks` actor moving it `failed → end`), not in a status or column of its own.
...
died at (ADR-0025). A failure the check **declines** to heal (invariant 25's three guards) is the deliberate exception: it stays in `failed/`, since nothing is coming to fix it and it must keep reading as a problem where the operator looks. `healed/` — the queue, the `HEALED` status and its board column — is retired (ADR-0024); the queue is still built and still hydrated, into the `done` column, so tasks an older version left there don't vanish on upgrade. With no `failed-tasks`-driving process configured, `failed/` simply has no reader, exactly as before wiring one up. See ADR-0018 and ADR-0024.
25. **The check produces at most one fresh task per claimed failure, and claims any given failure exactly once.** Every claim it takes into the heal pipeline settles to `done/` in the same `evaluate()` call. Recursion is guarded by a marker (`data.heal`), not by construction: a heal task that itself fails **does** pass through `failed/` normally (board-visible) and the check's next tick declines it without a new `Observation`. So no failure is healed twice and nothing loops. The check also stamps its own `params.repository` — a plain field of the `failed-tasks` action, set in `processes/autoheal.json` like any other Process action param, the operator's own responsibility to fill in — onto the fresh task it fires, so the `heal` step gets a worktree the same way any ordinary agent step does (invariant #15) — `harness init` seeds that file with `params == {}`, so a freshly initialized root fires a repository-less heal task until the operator edits it. **Three guards close three distinct cycles.** `data.heal` stops a failed *heal* task from being healed. Once a filed issue also carries an ingested label (`harness:todo`) rather than only one a human reads, a *fix* task born from that issue can fail too — and it carries no `data.heal` — so the check additionally declines any failed task whose `data["body"]` holds the `<!-- harness-issue:… -->` marker (`MARKER_PREFIX`, exported from `drivers/github_issues.py` so the literal has one source and two readers; its spelling has already changed once). A third guard closes the PR-borne half of the same shape: an unblock task (`GithubUnhealthyPrsCheck`) or an automerge-review task (`GithubMergeableCheck`) minted from the harness's own open pull request carries neither a body nor `data.heal`, so the check also declines any failed task whose `data["source"]["kind"]` is `"pull-request-health"` or `"pull-request"` (`PR_BORN_SOURCE_KINDS`, built in `drivers/failed_tasks_check.py` from each owning driver's own `SOURCE_KIND` constant, so a rename in either driver can't silently disarm it). All three leave the task **in `failed/`** — `heal-failed` for the first, `heal-declined` (each with its own reason) for the other two — and yield no `Observation`. The rule is deliberately broader than self-heal: **the harness does not heal a failure of work it filed for itself**, which covers every `open-issue` consumer and every PR-minted consumer, not just the healer. One automated fix attempt per root failure; a failed fix waits for the operator, and stays on the board where the operator will see it (ADR-0024). A declined failure is claimed exactly once, to append one history line saying why nothing is coming to fix it; that line is also the marker (`_already_declined`, matching the check's own `failed-tasks` actor) that makes every later tick skip the candidate without claiming it — so `failed/` neither churns nor loops, even though it no longer drains to empty. See ADR-0018, ADR-0019 and ADR-0024.
...
28. **A task's workspace branch is `harness/<task.id>` unless `task.data["branch"]` overrides it.** The override exists for exactly one case (the `unblock-pr` workflow fixing an existing PR): `GitWorkspace.attach` checks out that *existing* branch instead of creating a fresh one from HEAD. Absent the key, every path is unchanged.
...
30. **No task's worktree directory is ever removed.** Nothing under `src/harness` calls `git worktree remove`/`prune`. Consequence: a harness-authored branch is always still checked out in its original task's own worktree, so `GitWorkspace.attach`'s branch-override path force-checks it out into a *second* worktree (`git worktree add --force <path> <branch>`, reusing the existing local branch — not `-B`, which git refuses to force-reset on a branch checked out elsewhere no matter how many times `--force` is passed). Safe because the original worktree is inert for as long as its task stays terminal — with one operator-driven exception: `TaskControl.resume` (ADR-0025) returns a terminal failure to the step it died at and so revives its worktree. For an ordinary task (no `task.data["branch"]`) that revival is unconditionally safe — `attach`'s plain `else` branch reattaches with the same unconditional `reset --hard HEAD` + `clean -fd` every backward edge already relies on; only the `data.branch`-override subset (an `unblock-pr` or automerge-review task, which `resumable_failure` also admits, since a declined one of those sits in `failed/` too) relies on invariant 31's ancestry-aware reattach, where a branch force-checked-out into a second worktree meanwhile is the residual risk. Deliberate — don't "fix" the `--force` away, and don't add worktree cleanup without re-checking this invariant.
31. **The branch-override's reused local ref is untrusted until reconciled with `origin`.** The shared `refs/heads/<branch>` only tracks `origin/<branch>` while every advance of the branch goes through a local commit+push in *some* worktree — `GithubUnhealthyPrsCheck`'s call to `GithubClient.update_branch` (the `github-unhealthy-prs` process action) breaks that by advancing the branch server-side with no local git touch at all. So immediately after the `--force`d `worktree add` in the reuse path, `GitWorkspace.attach` hard-resets the *new* worktree to `origin/<branch>`'s fetched tip before returning the handle. That reset targets the branch as checked out in the new worktree, not "elsewhere," so it isn't blocked by the guard invariant 30 relies on. Don't drop this reset — without it, a `behind`→`update_branch`→`dirty`→`unblock-pr` sequence on the same PR leaves the `unblock-pr` task's worktree stale and its final `push` fails as non-fast-forward. The same reconciliation runs again on **reattach** (the `elif override:` branch, when the worktree already exists from a prior attempt) — but there it is ancestry-aware, not unconditional: `attach` hard-resets to `origin/<branch>` only when local `HEAD` is behind or equal to it (nothing local to lose); when local `HEAD` is ahead — the `unblock → land` hand-off producing an un-pushed merge commit in the same worktree — it's left untouched; if the two have diverged, `attach` raises rather than silently discarding either side. Don't collapse this back to an unconditional reset — that's the bug `#86` fixed.
...
37. **A scheduled trigger owns its cadence via the `Clock`, not via a loop.** `poll()` gates on an occurrence identity — `floor(now / interval)` for an interval cadence, the most recent matching UTC timestamp `<= now` for a cron cadence (`CronSchedule.occurrence_at_or_before`, `ports/triggers.py`) — and returns `[]` cheaply between fires; the shared `source_interval` is only polling granularity, true for either cadence. A cron fire lands on the first poll at/after its occurrence, never exactly on it. The gate reads solely `Clock`, so it is `FakeClock`-testable and survives the poller's `sleep(0)` fast-path without re-firing.
...
(including by the autoupgrade path). And **not** a shell `export` either —
launchd hands the service almost no environment, so exporting it in your
terminal has no effect on the live service. There is no "off" value: `0` is the
*most* aggressive setting (archive everything settled on the next sweep, the
"clear the board now" case), so effectively disabling the sweep means a very
large window — `36500`, a century. A value that is unparseable or negative warns
to stderr and falls back to the default rather than failing the run.
...
- `drivers/failed_tasks_check.py` — `FailedTasksCheck(Check)`: the single reader of `failed/`, expressed as a Process action (the `failed-tasks` check). Claims each healable failure via the atomic `claim()`, retires it into `done/` in the same `evaluate()` (ADR-0024), and emits one `Observation` carrying the rendered failure report (reason + history) with `state_key` = the failed task id (`per-state` dedup). Stamps a `data.heal` marker on what it produces and declines a failed task already carrying one — left in `failed/` with one history line saying why, never re-observed (invariant 25). Registered as the `failed-tasks` check inside `app.build()` (it closes over the live `failed`/`done`/`events` queues). See ADR-0018 and ADR-0024
- `behaviors/open_issue.py` — `OpenIssueBehavior(ConsumerBehavior)`: the `open-issue` finisher kind, fully config-driven — no notion of healing, reviewing or any other purpose baked in. Reads a step's artifact for a fenced json array of `IssueDraft`s (`harness.issue_drafts.parse_drafts`) and opens each via `IssueTracker` (the worker, never the LLM — invariant 9/26). `from_step`'s presence picks the shape: given, it fully *replaces* the bound step's behavior (the heal workflow's agent-less `file-issue`, reading `heal`'s artifact); omitted, it *wraps* the step's own behavior (`inner` runs first, then its own artifact is filed) — the shape a future 0..N-issue architecture-review process uses. Three independent label knobs, all binding config: `label` is carried by every issue and scopes the idempotency marker search; `labels` is the **floor** — a guaranteed set carried by every issue this binding files, whatever the persona drafted; `allowed_labels` is the **ceiling** — the allowlist a draft's own per-issue labels are filtered against, so a hallucinated label can't 422 the step. Config sets both bounds, the agent picks in between. A label in `labels` is implicitly allowed (a draft suggesting one is a no-op, never reported as dropped — it is on the issue either way). Deliberately *not* a Process-level field: labels belong to the thing that files the issue, and a Process that files nothing would carry a silently inert setting. Both lists are validated at wiring time by `cli._label_list`, as a plain `ValueError` — so a malformed one drops that workflow with a warning under ADR-0022, never a new error path. The repo comes from `task.repository` via an injected `slug_for` (never an import of a driver — `behaviors/` may not import `drivers/`). Wired via `build()`'s `finishers=` override. See ADR-0016, ADR-0018
...
- `ports/process_admin.py` — `ProcessAdmin` (write-side counterpart of
  ...
  `failed-tasks`/`github-issues`/`github-unhealthy-prs` are as fully-defined as the
  ...
  actions this run accepts (`github-issues`/`github-unhealthy-prs` included), not
...
- `behaviors/unblock_pr.py` — `UnblockPrBehavior`: merges the base into the
  attached branch; a clean merge with nothing red commits without spending an
  agent call, a real conflict or a failing check runs the `unblock` persona
  over the triage brief (`data.body`) then the worker commits, excluding
  `.artifacts` since the branch may belong to a human. Two corrections the
  brief can need before it reaches the prompt: the check's `data.problem`
  records the conflict flag as it was *at scan time*, so when the run-time
  merge disagrees, `data.body` is amended (`CONFLICT_APPEARED` /
  `CONFLICT_GONE`, module constants — the first duplicates the check's own
  wording word-for-word because `behaviors/` may not import `drivers/`) on a
  `dataclasses.replace`d copy of the task, never by mutating it. And a
  conflicted merge the agent did **not** finish (`stuck`, or any non-`DONE`
  outcome) is `abort_merge`d rather than committed, following
  `LandingBehavior`'s precedent — a give-up must push nothing. A non-`DONE`
  outcome with a *clean* merge still commits, and **not** because there is no
  merge to abandon (`merge()` runs `git merge --no-commit --no-ff`, so a clean
  non-empty merge does leave `MERGE_HEAD` and a staged merge) — it is that
  `abort_merge` cannot distinguish that from the commoner "Already up to date"
  shape, where no merge is in progress and `git merge --abort` would fail a task
  whose only sin was a red check. The resulting two-parent commit is local only:
  `stuck` routes to `end`, nothing pushes, and the next task's worktree is
  `reset --hard origin/<branch>`. Any non-`DONE` outcome *also* applies the
  task's `data["give_up_label"]` to the pull request through an injected
  `PrLabeller` callable (wiring closes a `GithubClient` into it; `behaviors/`
  may not import `drivers/`), best-effort — that label is the only thing that
  can ever reach `harness:needs-human` on this path, since `stuck` moves no head
  sha and so spends no attempt, and it is also what stops the archived task
  being re-minted forever (ADR-0027). A task with no readable
  `data["source"]["base"]` raises `UnblockPrError` naming the key rather than a
  bare `KeyError`. Excluding `.artifacts`
  has a cost the ADR spells out: on the success path the write-up survives only
  until `land` reattaches the worktree and `clean -fd` deletes it (ADR-0027)
...
- `drivers/github_unhealthy_prs_check.py` — `GithubUnhealthyPrsCheck(Check)`: pull-request triage as a process `Check`, the retired `GithubConflictsCheck`'s successor and the exact complement of `GithubMergeableCheck`. Partitions the same open PRs by `mergeable_state`, deterministically and with no agent: `behind` is updated server-side (a side effect, no task); `dirty`, or any PR carrying a check-run whose conclusion is in `FAILING_CONCLUSIONS` (imported from `github_client.py`, not re-derived), emits one `Observation` per PR keyed `slug:pr:head_sha`, carrying a structured `data.problem` (conflict flag + tailed failing-check logs) and its rendering as `data.body`; `clean` is left to the mergeable check. `blocked`/`unstable`/`unknown` PRs with no failing check-run are skipped (row 7) — nothing an agent could fix, since `blocked` is ambiguous between a failed required check and a missing required review and only the check-run fetch, not `mergeable_state`, tells them apart. `SOURCE_KIND = "pull-request-health"`. A PR whose `head_repo` is not the repo being scanned (compared **casefolded** on both sides — `slug` comes verbatim from `remote.origin.url` while `head_repo` is GitHub's canonical `full_name`, so a differently-cased clone like `OnPaj/Harness_v2` otherwise failed the guard for *every* PR: no observations, no warning, a green board) — a fork, or one whose fork has been deleted and reads as no head repo at all — is the **first** guard, ahead of the skip and give-up labels, so it is never worked on *and* never labelled: its head branch lives in someone else's repo, so `data.branch` would name a same-named branch of the base repo (usually `main`) and the workspace would commit and push there, violating ADR-0009. Log tailing (`log_tail_lines`) happens here, never in the agent, so a multi-megabyte log never reaches a prompt; a PR that raises during triage is isolated and does not sink the rest of the tick, printing one `warning:` line to stderr rather than failing silently (with `head_prefix: ""` a systemic fault — a revoked token, a rate limit — hits every PR, and a silent loop would read as "nothing to fix"). The attempt budget is a rolling `harness:autofix-<n>@<sha7>` label on the PR itself, stamped with the head sha the count was written against; a label already naming the current head is **not** bumped and costs no label call, because harness's own dedup ledger keys on `head_sha` and drops the duplicate only after the label is written — so without the stamp every service restart burned an attempt on an unchanged head. Only a genuinely new head sha (a real fix push) spends one, and a legacy unstamped label names no head and therefore still bumps. The budget ends in `harness:needs-human` once spent — but only the *push* path can ever spend it: the agent's own give-up (`stuck`) moves no head sha, so the check stamps its configured label into the task as `data["give_up_label"]` and `UnblockPrBehavior` applies it on any non-`done` outcome, which is what makes a give-up terminal instead of re-minted every retention window (ADR-0027). The attempt is spent — and the PR marked seen — only *after* the brief is assembled, so a transient 5xx on a `check_run_log` fetch costs nothing. Registered as the `github-unhealthy-prs` action in `cli._process_check_factories`; `harness init` seeds `processes/unblock-pr.json` (plus `workflows/unblock-pr.json` and `agents/unblock.json`) with `head_prefix: "harness/"` — matching `automerge.json`, so a fresh root touches only harness-authored branches. The check can work every open PR of every registered repo (the operator's own `~/harness-root` does), but widening it is one field of that file and the operator's explicit act
...
- `drivers/fs_processes.py` — `FilesystemProcessRepository`: reads `processes/*.json` (the top-level authoring aggregate — a nested `trigger`/`action`/`target`/`sink`) and **compiles each into a `ScheduledTrigger`**, validating fast (`ProcessValidationError`). A Process is a compile-time concept only; nothing under orchestration names it. The `trigger` block's cadence validates `cron` the same way as `interval` — exactly one of the two, fail-fast (`field="trigger"` for the block itself being malformed or carrying both/neither, `field="interval"`/`field="cron"` for a present-but-invalid value). The `sink` field names the outbound destination (`none` or `slack`, still just `{"kind": ...}` — no params, the webhook URL is a secret and never enters a JSON file); a non-`none` sink is passed to the compiled trigger, which stamps `data.sink`. An optional `trigger.kind` is a read-tolerated reservation: `"schedule"`-only, never written by the admin. The module-level `compile_process` is the single validator both the repository (startup) and `FilesystemProcessAdmin` (the write-side admin driver, on submit) run — `ProcessValidationError` carries an optional `field` so the admin maps a compile failure onto the right form field. `build()` additionally guards against a cross-file `github-issues` label collision (invariant #39): every `label`/`claimed_label` pair is collected across the whole batch, and a value reused by two files fails the build naming both — static/exact-string only, `FilesystemProcessAdmin.write` (single-file) can't run it. See ADR-0015, ADR-0018
- `drivers/label_issue.py` — `LabelIssueBehavior(ConsumerBehavior)`: a finisher that *wraps* (not replaces) a step's own behavior — runs the wrapped behavior (typically `ClaudeCliBehavior`, e.g. a `product-manager` persona), then applies an outcome → label mapping to `task.data.source`'s GitHub issue via `GithubClient.add_label`. A missing `data.source` or an outcome absent from the mapping is a no-op with a note appended to `summary`, never an exception; routing is untouched (invariant #8). The first `ConsumerBehavior` under `drivers/` rather than `behaviors/` — it depends on `GithubClient`, a driver with no dedicated port, and `test_behaviors_import_only_ports_not_drivers` forbids that import from `behaviors/`; `drivers/` importing `drivers/` is already precedented (`github_issues_check.py`). Wired only in `cli.py`'s `_run` (registered as the `"label-issue"` finisher kind only when `GITHUB_TOKEN` is set). See ADR-0018
...
- **`TaskQueue`** — the inbox, the step queues, `done/` and `failed/` are all
  instances of the same port. Terminal states are simply queues that nobody
  consumes — with one exception: when an autoheal Process is wired, `failed/` gets
  exactly one reader (the `failed-tasks` Check), which retires each healable
  failure into `done/` and leaves the rest where it is (invariant 24, ADR-0024).
- **Self-healing is a Process, seeded and live on every root** (not opt-in, not a
  ...
  `failed/`: it claims each healable failure,
  retires it into `done/`, and fires a fresh task through the three-step `heal`
...
- **`RetentionReconciler`** is a further sibling, retiring a terminal task on
  ...
  one-per-tick like `MergeReconciler`. **`failed/` is deliberately not swept**
  (ADR-0024): a failure the harness declined to heal must keep reading as a
  problem exactly where the operator looks, and archiving it would also make it
  unrestartable, since `TaskControlService.restart` searches `failed/` and
  nowhere else. Step queues are never swept either — a task sitting in one is
  backlog, not garbage, however long it has sat there. **The Resume affordance
  has the retention window's lifetime**: `TaskControlService.resume` searches
  `done/` and `failed/`, so a healer-retired failure (which lands in `done` —
  ADR-0024) stops being resumable once this sweep archives it, while one in
  `failed/` stays resumable indefinitely. That is the accepted trade, not an
  oversight — raise `HARNESS_RETENTION_DAYS` if a longer resume window matters
  more than a short board. Unlike either sibling it
  needs no external checker, only the local queues and the clock, so `build()`
  always constructs it — there is no "not configured" state to gate on, unlike
  the `GITHUB_TOKEN`-gated pair above. Runs on the same `reconcile_interval` as
  the others; an archived task keeps its file in `archived/` and stays
  resolvable by id, like every other archival.
...
- **Automatic merging is a Process the operator opts into** (ADR-0023) — the
  ...
  of `github-unhealthy-prs`: the two partition the same open PRs by
  `mergeable_state` with no overlap — `behind` → update the branch
  server-side, `dirty` (unconditionally) or `blocked`/`unstable`/`unknown`
  carrying a failing check-run → the `unblock-pr` workflow, **literal
  `clean`** → the `automerge` workflow (`blocked`/`unstable`/`unknown` with no
  failing check-run go to neither — nothing an agent could fix). The `clean`
  arm is unconditional, not "green and clean": `github-unhealthy-prs` returns
  as soon as it sees `mergeable_state == "clean"`, before it ever looks at
  check-runs, so a `clean` PR with a failing *non-required* check-run still
  goes to automerge review, never to `unblock-pr`. `clean` is GitHub's own
  verdict that the PR merges cleanly with every *required* check green and
  *required* review present, so the harness never re-implements branch
  protection — an operator tightens the gate by tightening the repo's
  ...
  PR's own branch (invariant #28's `data.branch` override, the `unblock-pr`
  workflow's path) and returns `approve`/`reject`; only `approve` reaches `merge`, whose
...
- **A Process** is the operator's top-level authoring aggregate — the "tie it all
  ...
  **trigger** (cadence), an **action** (a named `Check` + params — the inbound
  ...
  seeds `processes/` with three files — `autoheal.json` (self-healing's own
  Process, invariant 25), `automerge.json` (ADR-0023, withheld by the
  workflow's `dry_run`) and `unblock-pr.json` (ADR-0027, withheld to
  harness-authored branches by `head_prefix: "harness/"`); the operator adds any others, each an independent
  file.
...
- **The `failed-tasks` check drains `failed/` of everything healable — not to
  empty; recursion is guarded by a marker, not by construction.** The check
  claims each healable failure once and retires it into `done/` (ADR-0024),
  stamping `data.heal` on the fresh heal task it produces. A heal
  task that itself fails passes through `failed/` normally, and the check
  declines any failed task already carrying `data.heal`: it stays in `failed/`,
  annotated once, so there is no "healing the healer" loop and no silent
  disappearance either. What is left sitting in `failed/` is therefore exactly
  the set the harness refused to fix — bounded and inert, since each is claimed
  at most once ever. `IssueTracker` is
  ...
  `FailedTasksCheck.evaluate()` settles a claim into `done/` *before* it
  ...
  the in-memory fake so the finisher runs harmlessly. A *second* marker guards
  a second cycle: a failed task whose `data["body"]` carries
  `<!-- harness-issue:… -->` is a fix attempt for an issue the harness itself
  filed, and gets a `heal-declined` note instead of being healed
  again. A *third* guard closes a third cycle, for tasks that carry neither a
  body nor `data.heal`: a failed task whose `data["source"]["kind"]` is
  `"pull-request-health"` or `"pull-request"` is an unblock or
  automerge-review task minted from the harness's own open pull request
  (`GithubUnhealthyPrsCheck`/`GithubMergeableCheck`), and gets a `heal-declined` note the same
  way. So a `heal-declined` card sitting in the `failed` column means "the
  automated fix didn't work, it's yours now" — not an error, and not something
  waiting on another tick.
...
- **A scheduled trigger's `dedup_key` is NON-constant — this is deliberate, don't
  ...
  GitHub issue, whose key is its stable number). The key is occurrence-keyed
  (`floor(now / interval)` for an interval cadence, `CronSchedule.occurrence_at_or_before(now)`
  for a cron cadence — one seam, `ScheduledTrigger._occurrence`), or state-keyed on
  `observation.state_key` for `per-state` — a `per-state` check with a `None`
  `state_key` is a usage error (every state would collapse into one), not a silent
  default. The clock-gate (`_last_occurrence`) is only an optimization that
  short-circuits the poller's `sleep(0)` re-poll within a run; the persisted
  `dedup_key` + seeded `_seen` is the cross-restart guarantee. And that guarantee is
  **at-most-once per occurrence**, NOT suppress-while-in-flight: `_seen` remembers
  "ever ingested", not "currently open", so `per-state` re-fires if a state recurs
  after its task has terminated and `per-interval` can't coalesce an unresolved
  backlog — a live-task-consulting mode is a deliberate follow-up.
- **Cron fields are UTC-only, always.** `"0 6 * * 1"` fires at 06:00 **UTC**, not
  06:00 in the operator's local timezone (Europe/Prague, currently UTC+1/+2) — there
  is no per-trigger `tz` field yet (a noted follow-up, see ADR-0018). The admin UI's
  cron field hint says so too, not only here — the whole point of this gotcha is that
  "06:00" silently meaning UTC is exactly the kind of surprise an operator authoring a
  Monday-morning automation would otherwise hit first.
...
- **A missing credential disables one Process; a wrong value still kills the
  ...
  `github-unhealthy-prs`, `github-mergeable`, `jira-issues` — when the environment
...
- **A multi-observation check left on the default `per-interval` dedup silently
  ...
  (per issue), `github-unhealthy-prs` (per unhealthy PR), `github-mergeable` (per
