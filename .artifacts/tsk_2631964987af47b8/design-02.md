...
> This is a fresh design pass for a retried attempt at this task (the prior
> attempt's `development` step timed out on the `claude -p` call before
> writing any code — nothing in the worktree needed to be undone, and nothing
> about the design itself was at fault). This document supersedes
> `design-01.md` for downstream steps; it restates the design in full against
> `plan-02.md` rather than pointing back at the superseded plan, and folds in
> `architecture-01.md`'s findings as settled decisions — the two "\[resolved\]"
> items architecture-01 promoted from optional to mandatory were already
> written into `design-01.md`'s component design verbatim, and are restated
> here as such (not as open items), plus the one thing architecture-01 added
> beyond the original design: an explicit import-boundary guard test for the
> new core module. No source file this design touches has changed since
> `design-01.md`/`architecture-01.md` were written (`git log` on every touched
> file stops before this task's artifact commits), so nothing here needed to
> be re-derived from scratch — it needed to be carried forward correctly.
...
changes — the same "free" render `plan-02.md` (Interfaces) already notes.

This document assumes `plan-02.md`'s FRs/data model as the spec (identical in
substance to `plan-01.md`'s, with the four open questions it left now recorded
as decisions) and fills in concrete signatures, call sequences, and the
implementation details the plan's data model deliberately leaves to design.
Items architecture-01 confirmed as real bugs requiring a fix are marked
**\[mandatory\]** rather than **\[resolved\]**, to make clear these are not
optional polish — a naive implementation of "leave the board" breaks the whole
board (`snapshot()`) or silently loses a task on crash (`recover()`) without
them, so both fixes must ship in the same commit as the feature that exposes
the gap they close, not as a follow-up.
...
- **`FakeForge.pull_request_state`** — reads the matching record from
  ...
  `tests/test_smoke_git.py`'s extension (plan-02.md, Dependencies and scope)
...
a comparison against `outcome`). Architecture-01 checked this specifically
...
the bug this feature fixes. Architecture-01 verified this against the current
source (`app.py:121-126`, `dispatcher.py:99-111`, `drivers/fs_queue.py:59-65`)
and confirmed it's a real gap, not a hypothetical one — this fix ships in the
same commit as `PrWatcher`'s wiring into `Harness`, not separately, since the
bug doesn't exist until `PrWatcher` claims from `done`. Fix:
`Harness.recover()` includes `self._done` in the queues it recovers:
...
already puts `recover()` first, verified.
...
(`projection.py:96-98`).** Architecture-01 confirmed this is a real,
verified-against-source bug, not a hypothetical one: an archived task sits in
`_tasks` with no entry in `_columns` (see (b)), so that line raises
`KeyError`, not just "fails to list the task." Unfixed, the very first
`snapshot()` call after any archive takes down the *whole* board, not just
that one task's listing — this makes the fix mandatory, and it must ship in
the same commit as `archive()`, not as a follow-up. It must tolerate a missing
column:
...
what a naive implementation of this design would get wrong — architecture-01
flags this explicitly as the one sequencing mistake that turns a listing bug
into an outage.
...
left it (plan-02.md, Out of scope / Open questions — issue-side signaling is
explicitly deferred and now recorded as decided, not merely proposed).
...
Architecture-01 flags this as something the original design didn't call out:
...
directly "just this once." This design therefore treats the guard test as
...
`models.py`), confirmed by architecture-01.
...
## Summary of what changed relative to `design-01.md`

Nothing in the signatures, schemas or call sequences changed — architecture-01
approved `design-01.md` verbatim and found nothing to alter. This document:

1. Restates everything against `plan-02.md` instead of the superseded
   `plan-01.md`.
2. Relabels the two recovery/consistency fixes (`snapshot()`'s `KeyError`,
   `Harness.recover()`'s missing `done` queue) from **\[resolved\]** to
   **\[mandatory\]**, matching architecture-01's promotion of them from
   optional to required, with the verified-against-source evidence carried
   forward (§4, §6a).
3. Adds §8, the import-boundary guard test for `pr_watcher.py` — the one item
   architecture-01 flagged as missing from the original design rather than as
   a fix to something already proposed.
