...
> This is a fresh planning pass for a retried attempt at this task (the prior
...
> about the plan itself was at fault). This document supersedes `plan-01.md`
> for downstream steps; it restates the plan in full rather than pointing back
> at it, and folds in the decisions `architecture-01.md` already validated
> against the source, so this retry doesn't re-litigate settled questions.
...
history are kept on disk (for audit / direct lookup); only its presence on the
board goes away.
...
The originating issue's "Notes" section floats two design axes and explicitly
leaves them open. This plan resolves both, matching what the previous
attempt's design/architecture already settled on (see Open questions, which
records these as **decided**, not merely proposed, since a full plan → design
→ architecture pass already checked them against the source and found no
objection):
...
None outstanding for this retry — the two axes the originating issue left open, plus the follow-on questions this plan's own data model raised, were already worked through in a prior plan → design → architecture pass on this same task (before an unrelated `development`-step timeout forced a restart) and are recorded here as decisions, not proposals:
